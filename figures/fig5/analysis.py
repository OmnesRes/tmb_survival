...
for cancer in labels_to_use:
    ...
    events = df['OS'].values
    test_idx, test_ranks, all_risks = pickle.load(open(cwd / 'figures' / 'fig4' / ('FCN_runs.pkl'), 'rb'))[cancer]
    labels = []
...
plt.savefig(cwd / 'figures' / 'fig5' / ('SKCM_model.pdf'))
...
ax.set_title(cancer)
ax.text(.0, .97, 'Logrank p-value=%.1E' % p_value, transform=ax.transAxes)
plt.savefig(cwd / 'figures' / 'fig5' / ('SKCM_tmb.pdf'))
