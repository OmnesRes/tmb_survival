...
for model in results:
    ...
    # overall_risks = np.mean(normed_risks, axis=0)
    overall_risks = np.mean([i - np.mean(i) for i in normed_risks], axis=0)
    # overall_risks = np.mean([i[:, 0] - np.mean(i[:, 0]) for i in results[model][1]], axis=0)

    ax.plot(np.sort(tmb), overall_risks, linewidth=2, label=model)
    ...
    # ax.set_title(model)
    # ax.set_xlim(np.log(.5), np.log(64))
    # ax.set_ylim(-4, 1.5)

ax.plot(tmb, sim_risks - np.mean(sim_risks))
ax.plot(tmb, tmb*.48 - np.mean(tmb*.48))
