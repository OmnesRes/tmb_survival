...
tmb, sim_risks, times_events = pickle.load(open(cwd / 'figures' / 'supp_fig3' / 'quadratic_data.pkl', 'rb'))
indexes = np.argsort(tmb)
times = np.array([i[0][0] for i in times_events])
events = np.array([i[1][0] for i in times_events])

test_idx, results = pickle.load(open(cwd / 'figures' / 'supp_fig3' / 'quadratic_data_runs_0.pkl', 'rb'))
...
for idx_test in test_idx:
    ...
    polynomial_risks.append(np.sum(t_tmb * np.array([cph.params_[0], cph.params_[1]]), axis=-1))
ax.plot(np.sort(tmb), np.mean([i - np.mean(i) for i in polynomial_risks], axis=0)[indexes], linewidth=2, alpha=.5, label='Polynomial Cox', color='#9467bd')
...
plt.savefig(cwd / 'figures' / 'supp_fig3' / 'quadratic_data.pdf')
