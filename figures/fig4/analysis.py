...
ax.set_yticklabels([label_dict.get(i, i) for i in labels_to_use])
...
plt.savefig(cwd / 'figures' / 'fig4' / 'model_hazards.pdf')
...
for index, i in enumerate(tmb_stats):
    ax.text(1, 1 - ((index + 1) / len(y_indexes)) + .03, round(i[2], 1), transform=ax.transAxes)
plt.savefig(cwd / 'figures' / 'fig4' / 'tmb_hazards.pdf')
...
ax.set_xticks(np.arange(0, 14000, 2000))
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
ax.spines['bottom'].set_linewidth(1)
...
plt.savefig(cwd / 'figures' / 'fig4' / ('SKCM_model.pdf'))
...
ax.set_title(cancer)
ax.text(.0, .97, 'Logrank p-value=%.1E' % p_value, transform=ax.transAxes)
plt.savefig(cwd / 'figures' / 'fig4' / ('SKCM_tmb.pdf'))
