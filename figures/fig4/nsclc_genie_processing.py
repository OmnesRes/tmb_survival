...
sample_table = pd.read_csv(open(cwd / 'files' / 'genie' / 'nsclc' / 'cancer_level_dataset_index.csv'), sep=',', usecols=usecols
                           )
...
samples = pd.read_csv(open(cwd / 'files' / 'genie' / 'nsclc' / 'cancer_panel_test_level_dataset.csv'), sep=',', usecols=usecols)
...
samples = samples.loc[samples['type'] == 'LUAD']
samples = samples.loc[~samples['panel'].isin(['VICC-01-SOLIDTUMOR', 'UHN-48-V1'])]

##only use samples that are present in the genie release 13
...
data = pd.read_csv(open(cwd / 'files' / 'genie' / 'nsclc' / 'regimen_cancer_level_dataset.csv'), sep=',', skiprows=0)
...
##use genie release 13, https://doi.org/10.7303/syn50678294
...
with open(cwd / 'figures' / 'figure_4' / 'nsclc_sample_table.pkl', 'wb') as f:
