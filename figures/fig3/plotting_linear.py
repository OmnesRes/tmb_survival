...
ax.plot(tmb, np.mean([i - np.mean(i) for i in cox_risks], axis=0), linewidth=2, alpha=.5, label='Cox', color='#2ca02c')
...
for model in ['FCN']:
    ...
    ax.plot(tmb, overall_risks, linewidth=2, alpha=.5, label=model, color='#ff7f0e')
...
plt.savefig(cwd / 'figures' / 'fig3' / 'linear_data_0.pdf')
